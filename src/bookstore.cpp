...
void Bookstore::CheckParameter(std::string &str, int &type) {
  if (str[0] != '-') {
    type = -1;
    return;
  }
...
void Bookstore::UpdateKeyBlock(const std::string &str, int index, int op) {
  int sz = str.size();
...
void Bookstore::GetBookInformation(std::string &ISBN, std::string &name,
...
int Bookstore::KeywordNumber(const std::string &str) {
...
bool Bookstore::FindKeyword(const Book &book, const std::string &key) {
